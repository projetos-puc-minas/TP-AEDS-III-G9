...
public class Arquivo<T extends Registro> {
    ...
    // -------------------------------------------------------
    // CORREÇÃO 1: Método readAll() adicionado
    // Era chamado por TagLivroDAO, EditoraDAO e LivroDAO
    // mas não existia — causava erro de compilação.
    // -------------------------------------------------------
    public ArrayList<T> readAll() throws Exception {
        ...
        while (arquivo.getFilePointer() < arquivo.length()) {
            ...
            // Inclui tanto ativos quanto excluídos na leitura bruta,
            // mas marca corretamente a lápide para que o chamador filtre
            T obj = construtor.newInstance();
            obj.fromByteArray(dados);
            obj.setLapide(lapide);
            obj.setTamRegistro(tamanho);
            lista.add(obj);
        }
        return lista;
